...
public class HumanFightBehavior : MonoBehaviour
{
	...
	void Update () { // grab products or lost products
		if(!opponentComponent.IsFighting() || opponentComponent.HasFallen()) {            
            EndTime = Time.time;
			FinishFight();
		}
		else {
			if (Input.GetKey(KeyCode.F)) { 
				humanComponent.AddDamage(opponentComponent.IsFighting() ? 0.5f : 0f); // add damage to itself
				...
				if (inventory.isFull[0] && humanComponent.isLost() && opponentComponent.IsFighting()) {
					LostProduct();
				}
			}
		} 
